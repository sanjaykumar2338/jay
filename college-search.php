	...
	if(!empty($feestype) &&  (!empty($tuitionMax) || !empty($tuitionMin))){
		...
		if($feestype=='averagenetprice'){
			...
			if (!empty($tuitionMin) && !empty($tuitionMax)) {
				...
				$ids_sql = "SELECT UNITID FROM `sfa1718_p2` WHERE `" . $column . "` >= '" . $tuitionMin . "' AND `" . $column . "` <= '" . $tuitionMax . "'";
				//$ids_sql_1 = "SELECT UNITID FROM `ic2018_ay` WHERE `CHG2AY3` >= '".$tuitionMin."' AND `CHG2AY3` <= '".$tuitionMax."'";

			} elseif (!empty($tuitionMin) && empty($tuitionMax)) {
				$ids_sql = "SELECT UNITID FROM `sfa1718_p2` WHERE `" . $column . "` >= '" . $tuitionMin . "'";
				//$ids_sql_1 = "SELECT UNITID FROM `ic2018_ay` WHERE `CHG2AY3` >= '".$tuitionMin."'";

			} elseif (empty($tuitionMin) && !empty($tuitionMax)) {
				$ids_sql = "SELECT UNITID FROM `sfa1718_p2` WHERE `" . $column . "` <= '" . $tuitionMax . "'";
				//$ids_sql_1 = "SELECT UNITID FROM `ic2018_ay` WHERE `CHG2AY3` <= '".$tuitionMax."'";
			}
			...
			if (!empty($tuitionMin) && !empty($tuitionMax)) {
				...
				$ids_sql = "SELECT UNITID FROM `ic2018_ay` WHERE `" . $column . "` >= '" . $tuitionMin . "' AND `" . $column . "` <= '" . $tuitionMax . "'";
				//$ids_sql_1 = "SELECT UNITID FROM `ic2018_ay` WHERE `CHG2AY3` >= '".$tuitionMin."' AND `CHG2AY3` <= '".$tuitionMax."'";

			} elseif (!empty($tuitionMin) && empty($tuitionMax)) {
				$ids_sql = "SELECT UNITID FROM `ic2018_ay` WHERE `" . $column . "` >= '" . $tuitionMin . "'";
				//$ids_sql_1 = "SELECT UNITID FROM `ic2018_ay` WHERE `CHG2AY3` >= '".$tuitionMin."'";

			} elseif (empty($tuitionMin) && !empty($tuitionMax)) {
				$ids_sql = "SELECT UNITID FROM `ic2018_ay` WHERE `" . $column . "` <= '" . $tuitionMax . "'";
				//$ids_sql_1 = "SELECT UNITID FROM `ic2018_ay` WHERE `CHG2AY3` <= '".$tuitionMax."'";
			}
...
<ul class="list-group" style="margin-bottom: 0px;"> 
  ...
  <!-- <li class="list-group-item list-group-results tandf list-link" href="#tution-btn">Tuition & Fees<i class="arrow_icon fa fa-caret-right" aria-hidden="true"></i></li>  -->
   ...
   <?php
    ...
    //  echo "<div class='upgradepremium college-search'><div class='upgrade-content'><h4>Upgrade to Premium to Access this Feature</h4>
	//  <p>After at least 5 students that you have invited join Isuriz, your account will be upgraded to Premium to unlock this feature.</p></div><div class='upgrade-action'><a class='upgrade-now-btn' href='dashboard-invite.php'>Upgrade Now</a></div></div>";
   ?>
     <li class="list-group-item list-group-results colg-advanced-search-btn adv-search-res ad-srch" >Advanced Search<i class="arrow_icon fa fa-caret-right" aria-hidden="true"></i></li>
	<ul class="list-group advanced-search-output advance-search-menu">
